...
function deleteCard(link) {
    ...
    // Remove the specific card from the list
    cardDeleted = false;
    const cardEntries = document.querySelectorAll('.listdetails');

    cardEntries.forEach(entry => {
        const cardNumberElement = entry.querySelector('div:nth-child(3) p'); // Adjusted selector
        if (cardNumberElement) {
            const cardNumberInEntry = cardNumberElement.textContent.trim().replace(/\s/g, '').slice(-4); // Extract last 4 digits
            console.log("Checking entry:", cardNumberInEntry);
            if (!cardDeleted && cardNumberInEntry === cardNumber.slice(-4)) {
    ...
    // Log to see if any card was removed from the list
    if (!cardDeleted) {
        console.log("No matching card found in the list!");
    }
